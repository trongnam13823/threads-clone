  ...
  return (
    <ColumnLayout className={className} pageStackName={pageStackName}>
      {hasColumns && initPath === paths.home ? (
        <ColumnHeader className='flex items-center justify-center gap-4'>
          <span className='font-bold'>{currentFeedName}</span>
          <DropdownMenu>
            ...
            return (
              (!hasColumns || path === initPath) && (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      'flex items-center justify-center font-bold text-(--text-secondary) active:opacity-65 max-md:h-(--nav-mobile-h) max-md:flex-1 max-md:border-b-2',
