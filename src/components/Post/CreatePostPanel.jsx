...
const types = [
  ...
  { id: 3, name: "Trang cá nhân mà bạn theo dõi", desc: "Trang cá nhân mà bạn theo dõi có thể trả lời và trích dẫn" },
  { id: 4, name: "Chỉ khi được nhắc đến", desc: "Trang cá nhân mà bạn nhắc đến có thể trả lời và trích dẫn" },
];
...
export function CreatePostPanel({ onClose, className, classMainContent, CloseElement }) {
  ...
  return (
    <div
      className={cn(
        "flex h-svh max-h-svh w-svw flex-col border border-(--lines-primary) bg-(--elevated-background) md:h-fit md:w-[620px] md:rounded-2xl",
        className
      )}
    >
      {/* Header */}
      ...
      <footer className="mt-auto flex h-20 shrink-0 items-center justify-between px-6">
        <DropdownMenu>
          <DropdownMenuTrigger className="text-(--secondary-text)">{type.desc}</DropdownMenuTrigger>
